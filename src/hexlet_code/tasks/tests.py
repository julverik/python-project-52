...
class TaskCRUDTest(TestCase):
    ...
    def test_task_delete(self):
        ...
        task = Task.objects.create(
            name="Задача для удаления", status=self.status, author=self.user
        )
    ...
    def test_task_delete_permission_denied(self):
        ...
        task = Task.objects.create(
            name="Чужая задача", status=self.status, author=other_user
        )
        response = self.client.post(reverse("tasks:delete", args=[task.id]))
        self.assertEqual(response.status_code, 302)
...
class TaskFilterTest(TestCase):
    ...
    def test_filter_by_status(self):
        ...
        response = self.client.get(reverse("tasks:tasks"), {"status": self.status.id})
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_executor(self):
        ...
        response = self.client.get(
            reverse("tasks:tasks"), {"executor": self.other_user.id}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Задача 1")
    ...
    def test_filter_by_label(self):
        ...
        response = self.client.get(reverse("tasks:tasks"), {"labels": self.label.id})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Задача 1")
        self.assertNotContains(response, "Задача 2")
